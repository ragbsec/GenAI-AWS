...
sys.path.append('../')
from util.tagging import standard_tags_kv
...
def create_aoss_policy_attach_bedrock_execution_role(collection_id, bedrock_kb_execution_role):
...
def create_policies_in_aoss(vector_store_name, aoss_client, bedrock_kb_execution_role_arn):
...
def interactive_sleep_for(condition: Callable[[], bool]):
    """
    Sleep for a while until the given condition is met.
    """
    interval = 10
    print(f"Waiting for condition `{condition.__name__}` to be met. Checking every {interval}s:")
    i = 0
    while True:
        if (i % (interval * 5)) == 0:
            # do a line break
