...
def gateway_target_cleanup():
    ...
    if not response.get('items'):
        print("ℹ️  No gateways found to clean up")
        return
    
    # Process each gateway
    for gateway_item in response['items']:
        gateway_id = gateway_item['gatewayId']
        ...
        # Wait for AWS to propagate target deletions
...
def policy_engine_cleanup():
    """Clean up AgentCore Policy Engine and associated policies"""
    ...
    try:
        ...
        if not engines:
            print("ℹ️  No Policy Engines found to clean up")
        else:
            for engine in engines:
                engine_id = engine.get('policyEngineId')
                engine_name = engine.get('name', 'unknown')
                print(f"🗑️  Deleting Policy Engine: {engine_name} ({engine_id})")
                
                # Delete all policies in the engine first
                try:
                    policies_response = agentcore_client.list_policies(policyEngineId=engine_id)
                ...
                agentcore_client.delete_policy_engine(policyEngineId=engine_id)
                print(f"✅ Policy Engine {engine_name} deleted")
                
    except Exception as e:
        print(f"⚠️  Error during policy engine cleanup: {e}")
    
    # Clean up SSM parameters
    print("🗑️  Cleaning up Policy SSM parameters...")
    delete_ssm_parameter("/app/customersupport/agentcore/policy_engine_id")
    delete_ssm_parameter("/app/customersupport/agentcore/policy_engine_arn")
    print("✅ Policy SSM parameters cleaned up")


def evaluation_config_cleanup():
    """Clean up AgentCore Online Evaluation configurations"""
    agentcore_client = boto3.client("bedrock-agentcore-control", region_name=REGION)
    
    try:
        response = agentcore_client.list_online_evaluation_configs()
        configs = response.get('onlineEvaluationConfigs', [])
        
        if not configs:
            print("ℹ️  No Online Evaluation Configs found to clean up")
        else:
            for config in configs:
                config_id = config.get('onlineEvaluationConfigId')
                config_name = config.get('onlineEvaluationConfigName', 'unknown')
                print(f"🗑️  Deleting Online Evaluation Config: {config_name} ({config_id})")
                agentcore_client.delete_online_evaluation_config(
                    onlineEvaluationConfigId=config_id
                )
                print(f"✅ Evaluation config {config_name} deleted")
                
    except Exception as e:
        print(f"⚠️  Error during evaluation config cleanup: {e}")
